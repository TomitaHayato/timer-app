...
export const getTotalRecords = async(userId: string) => {
...
// 指定された日の記録を取得
export const getRecordsByDate = async(userId: string, daysAgo: number = 0) => {
...
// 指定された週の記録を取得
export const getRecordsByWeek = async(userId: string, weeksAgo: number = 0) => {
...
// 指定された月の記録を取得
export const getRecordsByMonth = async(userId: string, monthsAgo: number = 0) => {
...
const _getSummarizedRecordsByPiriod = async(userId: string, start: Date, end: Date) => {
